...
<details>
  <summary><i>Expand for Seafowl implementation details</i></summary>

#### Seafowl implementation

```js
sha256 = require('https://cdnjs.cloudflare.com/ajax/libs/js-sha256/0.9.0/sha256.js')
```

To (ab)use HTTP caches (browser, Varnish, CDNs), Seafowl supports running queries by passing the query hash in the URL and the actual query in the request header (with support for URL-encoding unprintable and non-ASCII characters) or a GET request body (but that isn't supported by `fetch()`).
